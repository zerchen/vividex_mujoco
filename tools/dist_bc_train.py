...
def main():
    ...
    for epoch in range(trainer.start_epoch, cfg.end_epoch):
        ...
        for itr, (inputs, targets) in enumerate(trainer.batch_generator):
            ...
            loss, actions = trainer.model(inputs, targets, 'train')
    ...
    video_list = cfg.test_video.split('/')
    env_list = []
    cfg_list = []
    data_traj = []
    object_name_list = []
    ...
    for env_idx in range(len(env_list)):
        ...
        for test_idx in range(100):
            ...
            while not done:
                ...
                action = trainer.model(cur_input, mode='eval')[0].cpu().numpy()
                # render(writer, env.wrapped.physics)
        ...
        trainer.logger.info(f'SR {cur_env_name}: {success_rate_10} {success_rate_5} {success_rate_3}')
    ...
    trainer.logger.info(f'overall SR: {mean_success_rate_10} {mean_success_rate_5} {mean_success_rate_3}')
